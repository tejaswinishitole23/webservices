...
import in.rk.jaxws.interceptor.out.WriteInterceptor;
import in.rk.jaxws.model.ExtractedHeaderVO;
 ...
 *  */
@Service
@WebService(portName = "CalculatorSOAP", serviceName = "Calculator", endpointInterface = "in.rk.service.calculator.Calculator", targetNamespace = "http://www.rk.com/service/Calculator/")
public class CalculatorWS implements Calculator {
	...
	public CalculateDataResponse addOrMultiply(CalculateDataRequest request, MwdsHeaderInfo hdr,
		...
		case "+":
			response.setReturnValue(add(request.getNumbers()));
			break;
		case "*":
			response.setReturnValue(multiply(request.getNumbers()));
			break;
		default:
	...
	// this can be used when you need something beyond the Java Bean with request
	// details.
